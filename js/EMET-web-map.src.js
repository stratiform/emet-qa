...
console.log('EMET Web Map version ' + vers);
...
var map, stateControl, linkDict, histLinkDict, lookupDict, featureControl;
...
var createInfoHTML = function(props, pointType, state, text){
    ...
    // opening
    html += "<div class='sidebar-title'><h1 id='side-title'>" + name + "</h1></div>";
...
var createMap = function() {
	...
	featHtml += "<input type='radio' name='point-filter' value='EMET Hub' class='hubs'> EMET Hubs<br>";
	featHtml += "<input type='radio' name='point-filter' value='all' checked class='hubs'> EMET Hubs and Training Sites<br>";
    featHtml += "<input type='radio' name='point-filter' value='connex' class='hubs'> EMET Hubs and Training Sites with connections<br>";
    ...
    $(".dropdown-selection").click(function() {
        $(".dropbtn").html("Showing: " + $(this).prop("id"));
    });
    ...
    */
    ...
    // points
    getCartoJSON(account, "emet_data_dev", map);
...
;
var sidebarText = {
  'contact': 'For more information on this EMET program, contact:',
  'site': {
    'heading': 'Serviced by:',
